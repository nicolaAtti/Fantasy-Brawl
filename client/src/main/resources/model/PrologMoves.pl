...
% specMove(@Name,@MoveType,@DamageType,@BaseValue,@ManaCost,@ModifiersList,@AfflictionsList,@RemovedAfflictions,@NOfTargets)
%
% Author: Nicola Atti

spec_move('Lay On Hands','Spell','StandardHeal',50,15,[],[],[],1).
spec_move('Censure','Spell','Nothing',0,30,[],['Silenced'],[],1).
spec_move('Holy Smite','Melee','StandardDamage',30,25,[],[],['Asleep'],1).
spec_move('Bolster Faith','Spell','Nothing',0,40,['Bolstered Faith'],[],[],4).
spec_move('Poison Vial','Ranged','StandardDamage',20,15,[],['Poisoned'],['Asleep'],1).
spec_move('Flash Grenade','Ranged','Nothing',0,20,[],['Blinded'],['Asleep'],4).
spec_move('Backstab','Melee','StandardDamage',60,15,[],[],['Asleep'],1).
spec_move('Preparation','Spell','Nothing',0,20,['Prepared'],[],[],1).
spec_move('Second Wind','Spell','Percentage',110,20,[],[],[],1).
spec_move('Sismic Slam','Melee','StandardDamage',40,15,[],[],['Asleep'],4).
spec_move('Skullcrack','Melee','StandardDamage',30,20,[],['Stunned'],['Asleep'],1).
spec_move('Berserker Rage','Spell','Nothing',0,30,[],['Berserk'],[],1).
spec_move('Freezing Wind','Spell','StandardDamage',50,20,[],['Frozen'],['Asleep'],1).
spec_move('Hypnosis','Spell','Nothing',0,30,[],['Asleep'],[],1).
spec_move('Concentration','Spell','Nothing',0,15,['Concentrated'],[],[],1).
spec_move('Flamestrike','Spell','StandardDamage',40,25,[],[],['Asleep'],4).

spec_move('Shield Bash','Melee','StandardDamage',10,20,[],['Silenced'],['Asleep'],1).
spec_move('Adrenaline Rush','Spell','Nothing',0,15,['Sped Up'],[],[],1).
spec_move('Iron Will','Spell','Nothing',0,15,['Will Of Iron'],[],[],1).
spec_move('Mighty Slam','Melee','StandardDamage',40,25,[],[],['Asleep'],1).
spec_move('Trip Attack','Melee','StandardDamage',15,15,['Slowed'],[],['Asleep'],1).
spec_move('Sunder Defenses','Melee','StandardDamage',5,15,['Sundered Defences'],[],['Asleep'],1).

spec_move('Chrono Shift','Spell','StandardDamage',15,30,[],[],['Asleep'],4).
