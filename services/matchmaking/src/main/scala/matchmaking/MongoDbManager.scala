...
object MongoDbManager {
  ...
  def getTicket: Future[Int] = {
    casualQueueCollection
      .findOneAndUpdate(
        filter = Filters.equal(fieldName = "_id", value = CasualQueue.TicketsDocumentId),
  ...
  def takePlayerFromQueue(ticket: Int): Future[(String, Set[String], String, String, Boolean)] = {
    casualQueueCollection
      .findOneAndDelete(
        filter = Filters.equal(fieldName = "_id", value = ticket))
      .map(
        document =>
  ...
  def removePlayerFromQueue(playerName: String): Future[DeleteResult] = {
    casualQueueCollection
      .deleteOne(
        filter = Filters.equal(CasualQueue.PlayerName, playerName))
      .head
  }
  ...
  /** Notifies that a player has left the game while waiting for an opponent.
    ...
    */
  def notifyPlayerLeftTheGame(playerName: String): Future[UpdateResult] = {
    casualQueueCollection
      .updateOne(
        filter = Filters.equal(CasualQueue.PlayerName, playerName),
